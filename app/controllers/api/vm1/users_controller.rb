class Api::Vm1::UsersController < Api::Vm1::ApplicationController
  ...
  before_action :authenticate_api, only: %i[payment_history email_and_notification update_profile get_user_by_id]
  before_action :authenticate_user, only: [:update_registration_plan]

  def contact_us
    begin
      @contact_us = ContactUs.new(contact_us_params)
      @response = if @contact_us.save
                    { code: '0', status: 'Success', message: 'Successfully contact created', contact_us: @contact_us }
                  else
                    { code: '1', status: 'Error', message: @contact_us.errors }
                  end
    rescue Exception => e
  ...
  def email_and_notification
    ...
    api_response = begin
      ...
      { code: '0', status: 'Success', message: 'Email and notification settings are successfully saved', email_and_notification: email_and_notification.as_json(except: %i[created_at updated_at]) }
                   rescue Exception => e
  ...
  def update_registration_plan
    api_response = if params[:registration_plan] == 'Educational' && FreeMember.find_by_email(@object.email).blank?
  ...
  def user_params
    ...
    @object.as_json(except: %i[created_at updated_at id])
  end
  ...
  def authenticate_user
    ...
    render(json: { code: '-1', status: 'unauthorize request' }) && return if @object.nil?
  end
